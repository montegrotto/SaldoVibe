...
| R-011 | Åtkomstkontroll och ansvarsfördelning | Känsliga åtgärder kräver korrekt behörighet och skälsloggning | Åtgärds-rollmatrisen i `bookkeeping/compliance_policy.py` (14 åtgärdsnycklar) upprätthålls via `@require_compliance_action` på samtliga 25 HTTP-endpoints med känslig åtgärd (periodlås, årsradering, bokslut, företagsradering, SRU/SIE4/exportpaket, SIE-import, momsstängning/eSKD, AGI-rapportering, compliance-dashboard, verifikationsserier); `restore.dry_run` och `gdpr.erase` är management-kommandon utan HTTP-yta. Kvarvarande `is_staff`/`is_superuser`-kontroller (`company_scope.py`, `views/companies.py`) är tenant-medlemskapsavgränsning, dokumenterat i `docs/compliance/role-matrix.md` | Implemented | Inget öppet gap (rollmatrisutrullningen slutförd 2026-08-24, sista spärrtestet tillagt 2026-09-07) | Ny känslig endpoint → lägg till nyckel i `ACTION_ROLE_MATRIX`, dekorera vyn och lägg till spärrtest i samma ändring | Varje åtgärdsnyckel med HTTP-yta har ett blockerad-för-icke-finance_admin-test, t.ex. `test_period_lock_actions_require_finance_admin_role`, `test_close_and_eskd_export_are_blocked_for_non_finance_admin`, `test_compliance_dashboard_is_blocked_for_non_finance_admin`, `test_plain_user_is_blocked_from_starting_an_export`, `test_plain_user_is_blocked_from_voucher_series_settings` | Användare utan finance_admin-roll nekas varje åtgärd i matrisen med felmeddelande och redirect; rollmatrisdokumentet matchar koden | P1 |
